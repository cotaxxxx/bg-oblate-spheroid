...
def main():
    ...
    syn=P.evaluation_record(0,0,0,Fraction(0),Fraction(1,64),Fraction(5,8),Fraction(1007,1600),2048,
        {"outcome":"nonfinite","stats":{"four_group_width_max":{"series":[arb(1),arb("nan"),arb(2)],"direct":[arb(3),arb(4)]}}})
    with tempfile.TemporaryDirectory() as d:
        ...
        assert all(isinstance(x,str) for x in rows[3]["stats"]["four_group_width_max"]["series"])
        assert all(isinstance(x,str) for x in rows[3]["stats"]["four_group_width_max"]["direct"])
    with tempfile.TemporaryDirectory() as d:
