# D-OB P2 CERTIFICATION SPECIFICATION V1 — H > 0 ON THE QUARTER MERIDIONAL DOMAIN
...
A cell is **regular** if an interval evaluation over `C x [lambda_lo, lambda_hi]` proves `|x - c| >= 2R` (far column: and also `|x - cbar| >= 2R`). Every other cell is **cut**. In the far column, `A_p` (resp. `A_pbar`) is the set of cut cells failing the test for `c` (resp. `cbar`); a cut cell may lie in both. On every regular cell and for every base point of `B` (far: and its mirror; near: every `p_s`), `D >= 2R - R = R > 0`.
...
- Every interval operation must be defined on its arguments. An undefined operation on a cell (for instance a `D^2` enclosure not strictly positive) makes that cell's evaluation fail; such a cell is treated as cut for the box, never as regular.
