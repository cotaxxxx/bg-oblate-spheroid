# C1b Pre-Run Amendment v2.5.1 — Gl t-Cell Splitting and Non-Finite Enclosure Fail-Closure

Status: PREDECLARED / NOT_EVIDENCE / MACHINE_NOT_RUN.
This supersedes v2.5 (blob b8226089…, commit 7f2ec7ee) in one respect only: §4 there allowed a single
gating-side edit (the ROOT_MV header), which contradicted the §3 estimate values — those require the
estimates() root-cost term to be updated as well. §4 below names both gating edits. Everything else,
including every constant and control, is byte-identical in substance to v2.5.
...
- The gating-side changes are exactly two, both required and both made identically in the producer and
  checker gating files: (i) the ROOT_MV header block gains "gl_panels_per_cell" and "gl_t_cells"
  (mirroring the gt entries), replacing "gl_panels"; and (ii) in estimates(), the root-cost expression
  replaces the term kernel.ROOT_GL_PANELS by kernel.ROOT_GL_T_CELLS * kernel.ROOT_GL_PANELS, so that the
  printed estimates equal the §3 values. No other gating behaviour changes. Note that this changes the
