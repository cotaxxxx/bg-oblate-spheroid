...
def safe_stats(x):
    ...
    if isinstance(x, arb): return "NONFINITE" if not x.is_finite() else x.str(40)
    if isinstance(x, dict): return {str(k):safe_stats(v) for k,v in x.items()}
    if isinstance(x, list): return [safe_stats(v) for v in x]
    raise TypeError(f"unsupported stats value: {type(x).__name__}")
