...
def _numeric_dependency_closure():
    seen=set(); stack=list(C1B_NUMERIC_ROOTS); package="producer"
    while stack:
        rel=stack.pop()
...
def numeric_import_closure_controls():
    ...
    try:
        C1B_NUMERIC_ROOTS=roots_old+("producer/global_axial_c1a_producer.py",)
        try: numeric_import_closure_preflight()
...
def endpoint_light_controls():
    ...
    for ulo,uhi in ((Fraction(1,10),Fraction(1,5)),(Fraction(2,5),Fraction(1,2)),
        ...
        for glo,ghi in ((Fraction(1,5),Fraction(3,10)),(Fraction(3,5),Fraction(7,10))):
            ...
            old=base._R_bundle(u,g,{"series":0,"direct":0,"series_hits_moving_u0":0,"chart_unresolved":0})[0]
...
def v282_preflight_controls():
    ...
    c1 = ('q = _positive_q_box(mu, t, lam, q_raw)' in src_glam and
          'sq = q.sqrt()' in src_glam and
          'gamma = lam * A / (w * sq)' in src_glam and
          'h = mu + l2 * d' in src_glam and
          'u = base._unit_nonnegative(e * h * h / (w2 * q))' in src_glam and
          '_gamma' in src_glam and '_u' in src_glam and '_sq' in src_glam and
          src_glam.count('inv_wq32 = _positive_inv_wq32') == 1 and
          'L = lam * inv_wq32' in src_glam and
          'regular_rg_term = -(gamma * R - 1) * gamma * e * k * inv_wq32' in src_glam)
    helper_src = inspect.getsource(_positive_q_box)
    helper_shape = ('t0 = t_lo if mu0 < t_lo else t_hi if mu0 > t_hi else mu0' in helper_src and
                    'q_lo = min(q_endpoint(mu_lo), q_endpoint(mu_hi))' in helper_src and
                    'd2' not in helper_src and 'd_lo' not in helper_src and 'd_hi' not in helper_src)
    ...
    q_impl = _positive_q_box(mu_box, t_box, lam_box, q_raw)
...
def attempt(slab, previous_root):
    tc, mode, predictor_work, predictor_nonfinite = choose_predictor(slab, previous_root)
    if predictor_nonfinite is not None:
        work = {"predictor": predictor_work, "tube": 0, "root": 0, "exterior": 0}
        rec = {"mode": None, "predictor_only": True, "predictor_nonfinite": [predictor_nonfinite]}
        return False, rec, None, None, work, "PREDICTOR_NONFINITE"
...
def diagnostic_controls():
    ...
    try:
        ...
        # predictor
        g_box=guard; pout=attempt(slab,None)
...
def v212_preflight_controls():
    ...
    print("C1B_V212_C1","PASS" if c1 else "FAIL","outcome","a" if ok else "b","root",root,"reason",reason,"work",work,"worst_step1_Gt_upper",worst);
    ...
    c5=len(top)==16 and all(arb(x["upper"])<0 for x in top); print("C1B_V212_C5_SWEEP_TOP","PASS" if c5 else "FAIL","lambda",(ll,lr),"work",131072,"enclosures",top)
