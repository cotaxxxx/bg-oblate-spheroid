...
def verify(path,out=None):
    ...
    if out:
        Path(out).write_text(canonical({"status":"PASS","leaves":rec})+"\n")
    return rec
def main():
    p=argparse.ArgumentParser(); p.add_argument("sealed_ledger"); p.add_argument("--output"); a=p.parse_args(); x=verify(a.sealed_ledger,a.output); print("PASS",len(x))
